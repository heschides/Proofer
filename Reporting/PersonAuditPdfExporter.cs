...
public sealed class PersonAuditPdfExporter
{
    ...
    public byte[] Generate(
        Person person,
        IReadOnlyList<PersonVersionDto> versions,
        Agency agency,
        User requester,
        ...
        DateTime generatedAtUtc)
    {
        ...
        certification.AddText(
            ...
            "It contains profile values recorded after each successful Sati change. Related records " +
            "such as notes, contacts, forms, assessments, and billing items maintain separate histories.");

        return document;
    }

    private static void AddHeaderAndFooter(Section section, Agency agency, Person person)
    {
        ...
        DateTime generatedAtUtc)
    {
        ...
        notice.AddText(
            ...
            $"(user {requester.Id}). Handle according to agency privacy, retention, and disclosure policy.");
    }
    ...
    private static void AddVersion(Section section, PersonVersionDto version)
    {
        ...
        banner.AddFormattedText(
            $"Version {version.Version} - {Safe(DisplayChangeKind(version.ChangeKind))}",
            TextFormat.Bold);

        var meta = section.AddParagraph(
            $"{version.ChangedAtUtc:yyyy-MM-dd HH:mm:ss} UTC  |  {Safe(version.ActorDisplayName)} " +
            $"(user {version.ActorUserId})  |  Request {Safe(version.CorrelationId)}");
    ...
    private static void AddChange(Section section, PersonFieldChangeDto change)
    {
